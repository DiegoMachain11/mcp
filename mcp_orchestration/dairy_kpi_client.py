...
class DairyKPIClient:
    ...
    def _get_kpi_list(self) -> pd.DataFrame:
        """Return a copy of the KPI catalog with alias metadata."""
        return self._kpi_catalog.copy()

    def _make_alias(self, description: str) -> str:
        normalized = unicodedata.normalize("NFKD", str(description))
    ...
    def _select_kpis(self, selected_kpis: Optional[Sequence[str]]) -> pd.DataFrame:
        kpis = self._get_kpi_list()
        print("All kpis = ", kpis)
        if not selected_kpis:
            return kpis

        raw_values = [str(item) for item in selected_kpis if item]
        if not raw_values:
            return kpis

        alias_targets = {self._make_alias(value) for value in raw_values}
        code_targets = {value.strip() for value in raw_values}

        filtered = kpis[
            kpis["alias"].isin(alias_targets)
            | kpis["Code"].astype(str).isin(code_targets)
            | kpis["Description"].str.lower().isin({v.lower() for v in raw_values})
        ]

        if filtered.empty:
            raise ValueError(
                f"None of the requested KPIs could be resolved: {sorted(raw_values)}"
            )

        print("Filtered kpis = ", filtered)
        return filtered
    ...
    def fetch_farm_kpis(
        ...
        language: str = "es",
        months: int = 13,
        selected_kpis: Optional[Sequence[str]] = None,
    ) -> pd.DataFrame:
        ...
        kpis = self._select_kpis(selected_kpis)
        all_data = []
        alias_map = {}
        ...
        if not all_data:
            if selected_kpis:
                logging.warning(
                    "No KPI data returned for farm %s (codes=%s)",
                    farm_code,
                    list(selected_kpis),
                )
                empty_cols = ["Date", *alias_map.keys()]
